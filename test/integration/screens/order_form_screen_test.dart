  ...
  Widget createWidgetUnderTest({String? orderUuid}) {
    return ProviderScope(
      overrides: [
        ...
        orderFormNotifierProvider(orderUuid)
  ...
  group('OrderFormScreen Widget Tests', () {
    testWidgets('shows loading indicator when state is loading',
      ...
      when(mockNotifier.state)
          .thenReturn(state); // Устанавливаем начальное состояние
      when(mockNotifier.stream)
          .thenAnswer((_) => Stream.value(state)); // Эмулируем стрим
    ...
    testWidgets('displays form fields in create mode', (tester) async {
      // Arrange
      final state = const OrderFormState(
        isLoading: false,
        isEditMode: false,
        initialOrder: null,
      );
    ...
    testWidgets('calls saveOrder on notifier when save button is tapped',
        (tester) async {
      // Arrange
      final state = const OrderFormState(isLoading: false, isEditMode: false);
      when(mockNotifier.state).thenReturn(state);
      when(mockNotifier.stream).thenAnswer((_) => Stream.value(state));
