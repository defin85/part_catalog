...
class SuppliersService {
  final AutodocApiClient _autodocClient;
  final ExistApiClient _existClient;
  final EmexApiClient _emexClient;

  SuppliersService({
    required AutodocApiClient autodocClient,
    required ExistApiClient existClient,
    required EmexApiClient emexClient,
  })  : _autodocClient = autodocClient,
        _existClient = existClient,
        _emexClient = emexClient;
  ...
  Future<Map<String, List<PartPriceModel>>> getAllPricesByArticle(String article) async {
    final results = await Future.wait([
      _autodocClient.getPricesByArticle(article).then(
            (prices) => MapEntry('autodoc', prices),
          ).catchError((_) => MapEntry('autodoc', <PartPriceModel>[])),
      _existClient.getPricesByArticle(article).then(
            (prices) => MapEntry('exist', prices),
          ).catchError((_) => MapEntry('exist', <PartPriceModel>[])),
      _emexClient.getPricesByArticle(article).then(
            (prices) => MapEntry('emex', prices),
          ).catchError((_) => MapEntry('emex', <PartPriceModel>[])),
    ]);
