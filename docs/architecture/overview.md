...
3. **Агрегаты (Aggregates)** и **корни агрегатов (Aggregate Roots)** - для обеспечения целостности данных
...
Компоненты: 
- Entities (сущности - аналоги справочников и документов в 1С)
- Value Objects (объекты-значения)
- Aggregates (агрегаты)
- Domain Services (сервисы предметной области)
- Domain Events (события предметной области)
Особенности: Не зависит от UI и источников данных, содержит правила и бизнес-логику
...
┌─────────┼────────────────┼┼───────────────┼────────────────┐
│         │   Domain Layer  ││               │                │
│      (Бизнес-логика, сущности, агрегаты, сервисы)         │
│  ┌──────▼──────┐  ┌───────▼──────┐  ┌─────▼───────┐        │
...
│         │ (Репозитории, DAO, Инфраструктурные сервисы)    │
│  ┌──────▼─────┐  ┌────────▼─────┐  ┌──────▼─────┐          │
