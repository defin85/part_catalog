...
Future<void> checkSupplierSettingsTable() async {
  ...
  try {
    ...
    for (final row in structure) {
      ...
      logger.i('  - $name: $type ($notNull)${defaultValue != null ? ' DEFAULT $defaultValue' : ''}');
    }
    ...
    if (allSettings.isEmpty) {
      ...
      for (int i = 0; i < allSettings.length; i++) {
        ...
        logger.i('  Has Encrypted Credentials: ${setting.encryptedCredentials != null}');
        if (setting.encryptedCredentials != null) {
          logger.i('  Credentials Length: ${setting.encryptedCredentials!.length} символов');
        }
        logger.i('  Last Check Status: ${setting.lastCheckStatus ?? 'Не проверялся'}');
        logger.i('  Created At: ${setting.createdAt}');
        logger.i('  Updated At: ${setting.updatedAt}');
        
        if (setting.additionalConfig != null) {
          logger.i('  Additional Config Length: ${setting.additionalConfig!.length} символов');
